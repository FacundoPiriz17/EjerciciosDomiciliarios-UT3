public class Lista<T> implements ILista<T> {
    ...
    public boolean buscarBoolean(Comparable clave) {
        Nodo<T> actual = primero;
        while (actual != null) {
            if (clave.equals(actual.dato)) {
                return true;
    ...
    public void eliminarDuplicados() { //O(N elevado al cuadrado)
        Nodo<T> nodoActual = primero;

        while (nodoActual != null) {
            Nodo<T> anterior = nodoActual;
            Nodo<T> otro = nodoActual.siguiente;

            while (otro != null) {
                if (nodoActual.etiqueta.equals(otro.etiqueta)) {
                    anterior.siguiente = otro.siguiente;
                    otro = anterior.siguiente;
                } else {
                    anterior = otro;
                    otro = otro.siguiente;
                }
